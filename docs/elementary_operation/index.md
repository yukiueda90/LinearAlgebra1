...
#### <u>注意</u>
行列 $A$ と列ベクトル $\mathbf{x}$ に対し, $A\mathbf{x} = \mathbf{0}$ という形の式を**斉次連立 $1$ 次方程式**という. 上の結果から, $A$ が正則でない場合, $\mathbf{x}=\mathbf{0}$ が必ず斉次連立 $1$ 次方程式の解となり, さらに $A$ が零行列でなければ, 解は無数に存在する.
