...
>> (背理法) &emsp; $F(r)$ が正則であると仮定する. いま, $r < n$ なので, $F(r)$ の 第 $r+1$ 行から第 $n$ 行の成分は全て $0$ である. 
...
上述のように, 行列に対して行基本変形を行うことで, 行階段簡約形 (単位行列を含む) に変形することができる. このような計算手順 (アルゴリズム) を**掃き出し法** (**row reduction**) もしくは **ガウスの消去法** (**Gaussian elimination**) という. 掃き出し法は, 先ほども紹介したように, 行列の階数を求めるテクニックとして応用できるが, 他にも連立 $1$ 次方程式の解法, もしくは逆行列を計算する方法としても有用である. これらを説明するため, まず拡大行列を導入する.
